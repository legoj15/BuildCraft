...
**Tiles**: *Most* block entities extend `TileBC_Neptune`, which provides item handler management, player tracking, and owner persistence. A documented set instead extend vanilla `BlockEntity` directly and re-implement the BC idioms by hand — the engines (`TileEngineBase_BC8` + subclasses, `TileDynamoMJ`), the fluid machines (`TileTank`, `TileDistiller_BC8`, `TileHeatExchange`, `TileFloodGate`), plus `TileSpringOil`, `TilePipeHolder`, `TileMarker`, and `TileLaser`. Skipping the heavyweight base is legitimate for these (they don't want its item/owner/player machinery), but there is currently **no shared block-entity base** between them, so the cross-version save/load directive, owner-NBT block, and `getUpdateTag`/`getUpdatePacket` sync pair are re-copied per file. Capability attachment uses NeoForge's `RegisterCapabilitiesEvent`.
