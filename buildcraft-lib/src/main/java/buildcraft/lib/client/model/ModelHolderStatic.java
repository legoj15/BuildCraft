...
public class ModelHolderStatic extends ModelHolder {
        ...
        for (String[] ar : textures) {
            if (ar.length != 2) {
                throw new IllegalArgumentException(
                    "Must have 2 elements (key,value) but got " + Arrays.toString(ar));
            }
            if (!ar[0].startsWith("~")) {
                throw new IllegalArgumentException(
                    "Key must start with '~' otherwise it will never be used!");
            }
    ...
    protected void onTextureStitchPre(Set<Identifier> toRegisterSprites) {
        ...
        try {
            ...
            BCLog.logger.warn("[lib.model.holder] Failed to load the model " + modelLocation
                + " because " + jse.getMessage());
        } catch (IOException io) {
            rawModel = null;
            failReason = "The model did not exist in any resource pack: " + io.getMessage();
            BCLog.logger.warn("[lib.model.holder] Failed to load the model " + modelLocation
                + " because " + io.getMessage());
        }
        if (rawModel != null) {
            if (ModelHolderRegistry.DEBUG) {
                BCLog.logger.info(
                    "[lib.model.holder] The model " + modelLocation + " requires these sprites:");
            }
            for (Entry<String, String> entry : rawModel.textures.entrySet()) {
                ...
                if (lookup == null || lookup.startsWith("#") || lookup.startsWith("~")) {
                    if (!allowTextureFallthrough) {
                        failReason = "The sprite lookup '" + lookup
                            + "' did not exist in any of the maps";
                        rawModel = null;
                        break;
                    }
                } else {
    ...
    private MutableQuad[] bakePart(JsonModelPart[] a) {
        ...
        for (JsonModelPart part : a) {
            for (JsonQuad quad : part.quads) {
                ...
                int attempts = 0;
                while (lookup.startsWith("#") && rawModel.textures.containsKey(lookup)
                    && attempts < 10) {
                ...
                if (lookup.startsWith("#") || lookup.startsWith("~")) {
                    if (allowTextureFallthrough) {
                        // Let the caller manually replace the sprite
                        sprite = null;
                    } else {
    ...
    private MutableQuad[][] getQuadsChecking() {
        if (quads == null) {
            if (unseen) {
                unseen = false;
                String warnText = "[lib.model.holder] Tried to use the model " + modelLocation
                    + " before it was baked!";
                if (ModelHolderRegistry.DEBUG) {
