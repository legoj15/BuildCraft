...
public enum EnumWireBetween {
    ...
    EnumWireBetween(Axis mainAxis, boolean xy, boolean yz) {
        ...
        boundingBox = new AABB(x1 / 16.0, y1 / 16.0, z1 / 16.0, x2 / 16.0, y2 / 16.0, z2 / 16.0);
        parts = getParts();
    }
    ...
    private EnumWirePart[] getParts() {
        Function<AxisDirection[], EnumWirePart> getPartFromDirections = directions ->
            Arrays.stream(EnumWirePart.VALUES)
                .filter(part -> part.x == directions[0] && part.y == directions[1] && part.z == directions[2])
