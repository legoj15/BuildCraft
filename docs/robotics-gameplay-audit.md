...
| V6 | Planter fed from a chest of 64 wheat seeds through a wooden pipe + station + command-built Provide Items gate (`trigger.true` → `station.provide_items`), farmland patch 2 blocks away | 7.1.x code: `takeSingle` — one seed per trip | UNFIXED build: the robot emptied the chest in one grab (64 → 0), planted one seed at t≈4 s and dropped the other 63 as a ground item (B4 reproduced); the idle Picker at the other station then flew over and collected the spilled stack | B4 confirmed in-game; re-run on the merged build below |
| V6b | Same planter rig, merged build (`b3947b0ac`+) | one seed per trip | chest 64 → 56 in 60 s, eight wheat planted on the 3×3 patch, zero ground items, robot cycles dock → plot → dock | B4 fixed and verified |
| V3b | `setblock air` on the pipe under the working planter, merged build | robot drops inert to the ground, sneak-wrench recovers | robot flew to the vanished dock, then sank to rest on the farmland (y 4.2) motionless, `mainAI` emptied (shut down), `currentStation` cleared, `linkedStation` kept as the memory of the lost home | B16 (both halves) fixed and verified |
