...
## Docking-station visuals

Two issues, found during the 2026-08-01 in-client comparison against `upstream/7.1.x` (1.7.10 ground truth):

1. **The state indicator never renders.** [PlugRobotStationRenderer.buildCap](../src/main/java/buildcraft/robotics/client/render/PlugRobotStationRenderer.java) places the decal quad at x = 4.01/16 facing WEST in WEST-canonical plug space — but x = 4/16 is the pipe-hull end of the post (x = 0 is the block edge, i.e. the actual outward face; convention confirmed in-client via blocker-plug placement, whose plate hugs the hull exactly as the model's 2–4.01px span predicts under this reading). The opaque 4px-deep baked post sits in front of the quad, so the available/reserved/linked dots are invisible from every angle. Verified in-client on 26.1.2: a freshly placed (registered, available) station shows no green anywhere; a docked robot shows no red. Fix: move the quad to x = −0.01/16, proud of the outward face. The four textures are byte-identical 7.1.x originals (green = available, yellow = reserved, red = linked corner dots).
2. **Shape doesn't match 1.7.10.** 7.1.x draws two boxes (world renderer `RobotStationPluggable$RobotStationPluggableRenderer`; same pedestal in `RobotStationItemRenderer`): an 8×8×1px plate hugging the pipe hull plus a ~2.2×2.2×3px post from the plate out to the block edge, with the WHOLE pedestal textured by the state icon (`pipeRobotStation[Reserved|Linked]`) — state readable from any angle, robot docks against the post end at the block face. The port ships a single 6×6×4px stub ([models/plugs/robot_station.json](../src/main/resources/assets/buildcraftunofficial/models/plugs/robot_station.json) + matching item model) textured with `robot_station_base` — a texture 7.1.x shipped but never referenced in its code. Decisions when fixing: (a) rebuild the two-box pedestal in both models; (b) how much state coverage to restore — outward-face decal only, decals on the post/plate faces, or a dynamic full-texture swap. Whatever the choice, keep the baked/dynamic split: `KeyPlugRobotStation`'s docs record why a state-varying baked key is a re-mesh perf bug.

Working notes from the same session: hand placement (Ph3 fix) and UP-face rotation both work; a docked robot covers the end cap (1.7.10 behaved the same); the docked robot's red charge LED is correct pre-Ph4 (no recharge AI yet).

## Plug model unification

Unify plug in-world geometry under vanilla `models/block/plug_*.json`. Today the plug rendering pipeline has three flavours: (a) blocker / power_adapter load BC-dialect JSON from `models/plugs/` via `ModelHolderStatic`, (b) timer / light_sensor / pulsar-base bake from hardcoded UVs in [PlugBakerSimpleItems.java:70-94](../src/main/java/buildcraft/silicon/client/model/plug/PlugBakerSimpleItems.java), and (c) lens / facade / gate are genuinely parametric and stay in Java. Migrate (a) and (b) to standard vanilla block-model JSONs loaded through the normal resource manager, with one generic rotating baker that pulls BakedQuads off a face and rotates them per `KeyPlug*.side`. Wins: resourcepacks can reshape *and* retexture any static plug with stock Minecraft JSON, the `models/plugs/` directory and `ModelHolderStatic` go away (verify nothing else uses the latter first), and surviving Java bakers become clearly-exceptional parametric cases. Tradeoff: non-trivial refactor across transport + silicon; per-element `shade` / extended UV tricks in the BC dialect need a NeoForge model-extension equivalent or to be dropped.
