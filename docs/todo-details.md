...
## Robot recipe Redstone Crystal

The shipped robot recipe (`data/buildcraftunofficial/recipe/robot.json`, `PPP/PRP/C C`) puts redstone dust in the centre. 7.1.x's key was `"crystalRedstone"`, which was a real item — `BuildCraftSilicon.redstoneCrystal`, Assembly Table recipe `buildcraft:redstoneCrystal`: 1 redstone block → 1 crystal at 10 000 000 RF (= 1 000 000 MJ; 7.1.x chipset RF÷10 matches the port's MJ chipset prices exactly). Ph7 misread it as ore-dict dust. Energy per robot: 7.1.x ≈ 1 160 000 MJ (crystal + 2 diamond chipsets); port today ≈ 160 000 MJ. At the laser's 4 MJ/t cap the crystal alone is ~3.5 h on one laser, ~52 min on four.

Art exists: `upstream/7.1.x:buildcraft_resources/assets/buildcraftsilicon/textures/items/redstoneCrystal.png` (+ a 1.8-era `models/item/redstonecrystal.json`). Options: (a) keep dust; (b) restore the crystal at an 8.0-scaled cost (~240 000–320 000 MJ, 3–4 diamond chipsets, so one robot ≈ 400–480 000 MJ — a "real power setup" gate without the 7.1.x multi-hour wait); (c) restore at the verbatim 1 000 000 MJ. If restored, it is Assembly Table only, lives in silicon (7.1.x placement), and the robot recipe keys on `#c:gems/redstone_crystal`-style tag only if another mod plausibly supplies one — otherwise the item id directly.
