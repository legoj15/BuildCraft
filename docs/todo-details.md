...
7.2.8 shipped "robots can now be named" (upstream `795c08681`, issue #3372); it reached 8.0.x/BC8 ancestry but **not** this port (zero `setCustomName` refs under `buildcraft/robotics/`) and is tracked in no doc. In modern MC the vanilla entity custom-name machinery may provide it almost free (name tag → `setCustomName` sync → nametag rendering) — needs one in-client check that `EntityRobot`'s entity-data setup doesn't suppress it. If it doesn't work free, it's a small parity item; verify during Ph7 (programming table) robot work.
