...
public class NodeFuncObjectObjectLongToObject<A, B, R> extends NodeFuncBase implements INodeFuncObject<R> {
    ...
    public NodeFuncObjectObjectLongToObject(String name, Class<A> argTypeA, Class<B> argTypeB, Class<R> returnType, IFuncObjectObjectLongToObject<A, B, R> function) {
        this(argTypeA, argTypeB, returnType, function, (a, b, c) -> "[ " + NodeTypes.getName(argTypeA) + ", " + NodeTypes.getName(argTypeB) + ", long -> " + NodeTypes.getName(returnType) + " ] " + name + "(" + a + ", " + b + ", " + c +  ")");
    }
    ...
    /** Shortcut to create a new {@link FuncObjectObjectLongToObject} without needing to create
     *  and populate an {@link INodeStack} to pass to {@link #getNode(INodeStack)}. */
    public FuncObjectObjectLongToObject create(INodeObject<A> argA, INodeObject<B> argB, INodeLong argC) {
        return new FuncObjectObjectLongToObject(argA, argB, argC); 
    }

    public class FuncObjectObjectLongToObject implements INodeObject<R>, IDependantNode, IFunctionNode {
        public final INodeObject<A> argA;
        public final INodeObject<B> argB;
        public final INodeLong argC;

        public FuncObjectObjectLongToObject(INodeObject<A> argA, INodeObject<B> argB, INodeLong argC) {
            this.argA = argA;
            this.argB = argB;
            this.argC = argC;
        ...
        public INodeObject<R> inline() {
            if (!canInline) {
                ...
                return NodeInliningHelper.tryInline(this, argA, argB, argC,
                    (a, b, c) -> new FuncObjectObjectLongToObject(a, b, c),
                    (a, b, c) -> new FuncObjectObjectLongToObject(a, b, c)
                );
            }
            return NodeInliningHelper.tryInline(this, argA, argB, argC,
