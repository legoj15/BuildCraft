...
/** Searches the station registry for the nearest station with an open request this robot's filter accepts,
 *  and RESERVES that request ({@code RobotRegistry.take} on its {@code ResourceIdRequest}) before
 *  terminating. Ported from 7.1.x {@code AIRobotSearchStackRequest}. */
public class AIRobotSearchStackRequest extends AIRobot {
    ...
    public AIRobotSearchStackRequest(IRobotAccess iRobot) {
        super(iRobot);
    }

    public AIRobotSearchStackRequest(IRobotAccess iRobot, IStackFilter iFilter, Collection<ItemStack> iBlackList) {
        this(iRobot);
    ...
    private boolean isBlacklisted(ItemStack stack) {
        for (ItemStack black : blackList) {
            if (StackUtil.isMatchingItem(stack, black)) {
                return true;
            }
        }
    ...
    private StackRequest getOrderFromRequestingStation(DockingStation station, boolean take) {
        for (StackRequest req : getAvailableRequests(station)) {
            if (!isBlacklisted(req.getStack()) && filter.matches(req.getStack())) {
                ...
                if (take) {
                    if (robot.getRegistry().take(req.getResourceId(robot.level()), robot.getRobotId())) {
                        return req;
                    }
                } else {
                    return req;
