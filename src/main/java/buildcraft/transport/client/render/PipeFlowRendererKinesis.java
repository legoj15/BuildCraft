...
import buildcraft.transport.pipe.flow.AbstractPipeFlowPower;
import buildcraft.transport.pipe.flow.AbstractPipeFlowPower.Section;

/**
 * Shared draw kit for the two kinesis flow renderers — {@link PipeFlowRendererPower} (MJ, the
 * {@code POWER_FLOW} sprite) and {@link PipeFlowRendererFE} (FE/RF, the {@code POWER_FLOW_OVERLOAD}
 * sprite). Both draw the same scrolling-cube geometry off {@link AbstractPipeFlowPower}'s display
 * state; only the sprite differs, so it is the single parameter.
 *
 * <p>The two enum renderers had drifted: the FE one built its side-flow box inline and left
 * {@code centrePower} un-clamped, so an overloaded RF pipe drove {@code centreRadius} negative,
 * inverted the box, and the cell-clipped scroll renderer skipped every cell — the connecting stems
 * rendered <em>invisible</em> while the centre cube stayed. Routing both through the clamped
 * {@link #sideFlowBox} (the MJ renderer's correct behaviour) fixes that; the shape is otherwise
 * unchanged.
 */
public final class PipeFlowRendererKinesis {
    ...
    /** Draws every side stem plus the centre cube into {@code consumer} using {@code sprite}. Node-agnostic:
     *  {@code pose} / {@code consumer} come from the {@code submitCustomGeometry} lambda on >=1.21.10 and from
     *  the immediate-mode buffer source on the older nodes. */
    static void drawAll(AbstractPipeFlowPower flow, float partialTicks, PoseStack.Pose pose,
                        VertexConsumer consumer, TextureAtlasSprite sprite) {
        double centrePower = 0;
        ...
        for (Direction side : Direction.values()) {
            ...
            renderSidePower(side, power[i], centrePower, offset, consumer, pose, sprite);
        }
        ...
        renderCentrePower(centrePower, offsetX, offsetY, offsetZ, consumer, pose, sprite);
    }
    ...
    private static void renderSidePower(Direction side, double power, double centrePower, double offset,
        VertexConsumer bb, PoseStack.Pose pose, TextureAtlasSprite sprite) {
        if (power < 0) {
            return;
        }
        if (sprite == null) {
    ...
    /** Builds the AABB for a single side's flow stem — the cuboid that extends from the centre cube out
     *  to the connected neighbour. Package-private for {@code PipeFlowRendererPowerGeometryTester}.
     *
     *  <p>{@code power} and {@code centrePower} are both displayPower / MJ ratios. They MUST be clamped
     *  to {@code [0, 1]} before computing the box: an overload (more power flowing than the pipe's
     *  maxPower per tick — e.g. five creative engines at 256 MJ each pushing through a 256 MJ/t diamond
     *  pipe) drives displayPower above MJ, which makes {@code centreRadius = 0.252 - 0.248 * centrePower}
     *  negative, which inverts the box (min &gt; max along the side axis), which makes
     *  {@link #renderScrollingBox}'s {@code pMinX >= pMaxX} guard skip every cell — leaving the connecting
     *  stems invisible while the centre cube still renders, i.e. visible gaps between adjacent pipes. */
