 ...
 *
 *  <p>7.1.x suppressed the whole tooltip for a blank robot — that half is kept as-is, except that the
 *  port's charge line stays (an empty-board robot can be placed here, so it can hold charge). */
public class ItemRobotTooltipTest extends VanillaSetupBaseTester {
    ...
    @Test
    public void aBlankRobotShowsOnlyItsCharge() {
        ...
        Assertions.assertEquals(1, tooltip.size(),
                "7.1.x suppressed the board description for a blank robot; the port keeps only its charge "
                        + "line. Got: " + tooltip);
    }
