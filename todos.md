...
- [ ] **Power unit display configuration** — umbrella of cosmetic knobs that shape how `LocaleUtil` renders energy values in machine readouts, ledgers, JEI labels, and chat. Status by sub-feature:
  ...
  - ⏳ **Large-number abbreviation** (`1.2k MJ`). A toggle that collapses readouts at or above 1000 into a compact suffixed form: `1,234 FE` → `1.2k FE`, `1,500,000 FE` → `1.5M FE`, `2,500,000,000 FE` → `2.5G FE`. Implementation sketch: extend `LocaleUtil.formatLong` (and optionally `formatDouble`) with a branch that, when the toggle is on AND the absolute value ≥1000, divides by the largest applicable power-of-1000 magnitude, formats the quotient via `formatDouble(value, 1)` so the configured decimal separator still applies, and appends the suffix. Decide whether the threshold belongs to the toggle or as a separate `IntValue` (1.12.2 used a hard-coded 1000; configurable threshold is overkill). Decide also whether MJ values participate — they rarely exceed a few thousand at runtime so the visual win is mostly for the FE side, but consistency argues for routing both through the same path. Skip for the JEI category power label specifically, where the recipe cost is the load-bearing detail and `80,000.0 MJ` truncating to `80.0k MJ` would lose the precision that distinguishes recipes.
