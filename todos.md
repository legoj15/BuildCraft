...
Last audited: 2026-05-23. The changelog block in `changelog.md` titled "Changes since 26.1.x Beta release 6" is authoritative for what has shipped since the previous audit; consult it before re-auditing this file.
...
### Advancements — 10 orphaned triggers
`minecraft:impossible` is the placeholder used for advancements granted from Java via `AdvancementUtil.unlockAdvancement(...)`. The following 10 advancements still have only an impossible-trigger JSON with no matching Java grant — pick a thematic in-code event for each and wire it up:
...
- [ ] **Split pipe textures onto a dedicated `buildcraftunofficial:pipes` atlas.** Would shrink the vanilla blocks atlas from 4096×2048 back to ~1024×1024 by moving the 400 `dye_replace`-generated dyed fluid-pipe variants (plus the 25 base fluid pipe sprites and the rest of `textures/pipes/`) off the vanilla atlas onto a dedicated one. (The atlas was 8192×4096 until the 2026-05-19 fix that removed the `gui` atlas source — see changelog — which took 523 stray vanilla GUI sprites plus BuildCraft's own 54 GUI sheets off the blocks atlas, shrinking it to 4096×2048: a quarter of the area, well within every GPU's texture-size limit. So this is now pure cleanup, not the low-spec-GPU concern it once was.) **Blocked by three independent vanilla-level constraints** (all empirically verified 2026-05-15 via a one-pipe-family spike):
